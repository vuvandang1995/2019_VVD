...
- **https**: The protocol used to access the Portal and the token/notification service.  **If Notary is enabled, bạn bắt buộc phải cấu hình https**
  ...
  ```
  ...
  hostname: harbor.dang.kites.rocks
  ...
  https:
    port: 443
    certificate: /etc/letsencrypt/live/harbor.dang.kites.rocks/fullchain.pem
    private_key: /etc/letsencrypt/live/harbor.dang.kites.rocks/privkey.pem
  ...
  ```
...
- Ở đây, tôi config storage backend sử dụng Google cloud storage (GCS).
  - Tạo 1 bucket(gcs)
  
  <img src="https://i.imgur.com/6Brqcva.png">
  
  - Tạo service account, tạo key file để có quyền truy cập vào storage đó
  
  <img src="https://i.imgur.com/msmaCx8.png">
  
  - Config trong **harbor.yaml**
  
  ```
  ...
  storage_service:
  gcs:
    bucket: harbor-dangvv-storage
    keyfile: /etc/registry/storage/learned-catcher-241105-71843b24043a.json
  ...
  ```
...
# IMPORTANT nếu bạn cấu hình storage backend là GCS
- Say khi chạy script `install.sh` xong, sẽ sinh ra file `docker-compose.yaml` trong thư mục harbor, hãy chỉnh sửa file đó ở service `chartmuseum`, mục `volume`, dòng `target`thành:
